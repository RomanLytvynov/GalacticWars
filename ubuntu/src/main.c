...
int init() {
    ...
    pack1 = (t_enemy **)malloc(4*sizeof(t_enemy *));
    for (int i = 0; i < 3; i++) {
    ...
    pack2 = (t_enemy **)malloc(5*sizeof(t_enemy *));
    for (int i = 0; i < 4; i++) {
    ...
    pack3 = (t_enemy **)malloc(4*sizeof(t_enemy *));
    for (int i = 0; i < 3; i++) {
...
int main () 
{
    int file = open("resource/text/high_score.txt", O_RDONLY);
    ...
    SDL_Texture *rey_cast = loadTexture("resource/images/rey_cast.png", ren_mainplane);
           ...
           if (pack4->on_display) {
                    ...
                    if (pack4->is_shoot && !pack4->burst) {
                        for (int i = pack4->y_enemy; i < 1000; i++) {
                                    renderTexture(rey_cast, ren_mainplane, pack4->x_enemy + 195, pack4->y_enemy + 330 + i, 10, 10);
                                    renderTexture(rey_cast, ren_mainplane, pack4->x_enemy + 195 - 90, pack4->y_enemy + 330 - 75 + i, 5, 10);
                                    renderTexture(rey_cast, ren_mainplane, pack4->x_enemy + 200 + 90, pack4->y_enemy + 330 - 75 + i, 5, 10);
                        }
                    }
                    else if (pack4->burst) {
